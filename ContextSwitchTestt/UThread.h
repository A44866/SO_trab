...
// The number of commutations between Threads.
//
static
DWORD NumberOfCommutations;
//
...
PUTHREAD ExtractNextReadyThread() {
	return IsListEmpty(&ReadyQueue)
		? MainThread
		: CONTAINING_RECORD(RemoveHeadList(&ReadyQueue),
			UTHREAD, Link);
}
...
VOID Schedule() {
	...
	NumberOfCommutations++;
	NextThread->State = RUNNING;
	ContextSwitch(RunningThread, NextThread);
	if (RunningThread->ToTerminate && RunningThread->EnableTermination) {
		UtExit(RunningThread->Result);
	}
}
...
BOOL UtDump(HANDLE tHandle, UT_INFO_THREAD info) {
	...
	info->StackUsage = (DWORD)(sizeof(thread->ThreadContext) * 100 / sizeof(thread->Stack));
	return TRUE;
}

VOID UtTerminateThread(HANDLE tHandle) {
	PUTHREAD thread = (PUTHREAD)tHandle;
	if (thread->EnableTermination == FALSE) {
		thread->ToTerminate = TRUE;
	}
	else {
		if (thread == UtSelf()) {
			UtExit(thread->Result);
		}
...
VOID UtDisableTermination() {
	RunningThread->EnableTermination = FALSE;
}

VOID UtEnableTermination() {
	RunningThread->EnableTermination = TRUE;
}
...
// Now  UtExit receives the thread result and save it in thread
// descriptor, so the descritor must remain alive
// until result is consulted by another thread
//
...
//
VOID InternalStart() {
	LONG res = RunningThread->Function(RunningThread->Argument);
	UtExit(res);
}
...
VOID __fastcall CleanupThread(PUTHREAD Thread) {
	free(Thread->Stack);
	// Now the thread descriptor must remain until
	// thread result is consulted
	//free(Thread);
}
...
BOOL UtGetThreadResult(HANDLE hThread, PLONG res) {
	...
	*res = thread->Result;
	// the thread descriptor does his job, destroy it
	free(thread);
	return TRUE;
}

//Returns the number of commutations between threads
DWORD UtGetSwitchCount() {
	return NumberOfCommutations;
}
//
...
// at the end of the ready queue.
//
HANDLE UtCreate64(UT_FUNCTION Function, UT_ARGUMENT Argument, CHAR * Name, SIZE_T SizeOfStack) {
	PUTHREAD Thread;
	SIZE_T size;
	Thread->Name = Name;
	if (SizeOfStack >= MINIMUM_STACK_SIZE) size = SizeOfStack;
	else size = STACK_SIZE;
	...
	Thread->ThreadContext = (PUTHREAD_CONTEXT)(Thread->Stack +
		size - sizeof(UTHREAD_CONTEXT) - sizeof(ULONGLONG) * 5);
	...
	Thread->State = READY;
	//
	Thread->EnableTermination = FALSE;
